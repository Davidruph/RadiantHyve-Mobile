...
class ToastyInfo {
  showToast({String? message, Color? backgroundColor, Gradient? backgroundGradient, icon, iconColor}) {
    if (isAppInForeground == AppLifecycleState.resumed) {
      Future.delayed(const Duration(milliseconds: 200), () {
        ...
        Flushbar(
          ...
          backgroundGradient: backgroundGradient ?? (backgroundColor == null ? color.buttonGradient : null),
          borderColor: Colors.white.withOpacity(0.5),
        ).show(Get.context!);
      });
    }
